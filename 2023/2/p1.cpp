...
int main(int argc, char** argv) {
    ...
    while(fgets(line, sizeof(line), fInput)) {
        ...
        if(blueCubes <= iBLUE_CUBE_MAX && redCubes <= iRED_CUBE_MAX && greenCubes <= iGREEN_CUBE_MAX) {
            std::smatch theMatch;
            std::regex_search(strLine, theMatch, regGameID);
            int id = boost::lexical_cast<int>(theMatch[1].str());
            iIDSum += id;
        }
